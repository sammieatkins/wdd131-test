...
function renderTasks(tasks) {
  ...
  // if (list.innerHTML == "") {
    ...
    tasks.forEach((task) => {
      // for each of them we need to add the HTML markup for a todo.
      list.innerHTML += `<li ${task.completed ? 'class="strike"' : ""}><p>${task.detail}</p>
          <div>
            <span data-function="delete">❎</span>
            <span data-function="complete">✅</span>
          </div>
        </li>`;
      // let task = `<li${item.completed ? 'class="strike"' : ""}><p>${item.detail}</p><div><span data-function="delete">❎</span><span data-function="complete">✅</span></div></li>`;
...
function newTask() {
  ...
  let todoObject = { detail: input, completed: false}
...
// we need to attach listeners to the submit button and the list. Listen for a click, call the 'newTask' function on submit and 
// call the 'manageTasks' function if either of the icons are clicked in the list of tasks.

let submit = document.querySelector("#submitTask");
